        ...
        {NAV_ITEMS.map(({ href, icon: Icon, label }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className={`relative flex flex-col items-center gap-0.5 w-16 py-1.5 rounded-xl transition-all active:scale-90 ${
                isActive ? 'text-[#4f46e5]' : 'text-[#9ca3af]'
              }`}
            >
              {isActive && (
                <div className="absolute top-0.5 w-1 h-1 bg-[#4f46e5] rounded-full blur-[1px] shadow-[0_0_4px_#4f46e5]" />
              )}
              <Icon
                className={`w-[22px] h-[22px] ${isActive ? 'drop-shadow-[0_0_8px_rgba(79,70,229,0.5)]' : ''}`}
